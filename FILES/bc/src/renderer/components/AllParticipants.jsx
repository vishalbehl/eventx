...
import {
  ...
  DialogContent, DialogActions, CircularProgress, Alert, Checkbox
} from '@mui/material';
...
export default function AllParticipants({ user }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await window.electronAPI.getLocalParticipants(eventId, cleanFilters);

      if (res.success) {
        setParticipants(res.participants || []);
        setTotalCount(res.participants.length);
      } else {
        showSnackbar(res.message || 'Failed to fetch data', 'error');
      }
    } catch (err) {
  ...
  // --- REFACTORED (Print logic now uses local data) ---
  const handlePrint = async (ids, type = 'badge') => {
    if (!ids || ids.length === 0) {
      showSnackbar('No participants selected for printing.', 'warning');
      return;
    }
    ...
    try {
      const eventId = user.assignedEventId;
      // Get participant details from the current state (already fetched locally)
      const participantsToPrint = participants.filter(p => ids.includes(p.id));

      // Generate JWT tokens for each participant (this still uses Electron API, which is correct)
      for (let i = 0; i < participantsToPrint.length; i++) {
        try {
          const res = await window.electronAPI.generateQRCode(participantsToPrint[i]);
          participantsToPrint[i].jwtToken = res.success ? res.token : participantsToPrint[i].regno;
        } catch {
          participantsToPrint[i].jwtToken = participantsToPrint[i].regno;
        }
      }

      // Use the local Electron API to get event details and templates
      ...
      const templateId = type === 'badge'
        ? eventRes.event.badge_template_id
        : eventRes.event.certificate_template_id;
      ...
      showSnackbar(err.message || 'An error occurred during PDF generation.', 'error');
    } finally {
