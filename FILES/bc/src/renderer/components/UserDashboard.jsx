...
// --- REFACTORED: Add the 'user' prop to access the event ID ---
export default function UserDashboard({ user }) {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  // --- REFACTORED: Fetch data from the local backend ---
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const res = await window.electronAPI.getLocalDashboardStats(user.assignedEventId);
        if (res.success) {
          setData(res);
        } else {
  ...
  // The rest of the component (chart data memoization and JSX) remains the same.
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h3" gutterBottom>User Dashboard</Typography>
